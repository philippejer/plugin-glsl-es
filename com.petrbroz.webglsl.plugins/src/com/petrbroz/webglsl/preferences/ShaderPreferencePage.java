...
public class ShaderPreferencePage extends FieldEditorPreferencePage
	...
	public ShaderPreferencePage() {
		super();
		setPreferenceStore(Plugin.getDefault().getPreferenceStore());
		setDescription(Messages.Label_ConfigurationDesc);
	}
	...
	protected Control createContents(Composite parent) {
		...
		label.setText(Messages.Label_Color);
		...
		color.addListener(new IPropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent event) {
				...
				if (token != null) {
					...
					TextAttribute att2 = new TextAttribute(ColorManager
		...
		data.verticalAlignment = GridData.VERTICAL_ALIGN_BEGINNING;
		label.setLayoutData(data);
		label.setText(Messages.Label_Bold);
		...
		data.verticalAlignment = GridData.VERTICAL_ALIGN_BEGINNING;
		label.setLayoutData(data);
		label.setText(Messages.Label_Italic);
		...
		data.verticalAlignment = GridData.VERTICAL_ALIGN_BEGINNING;
		label.setLayoutData(data);
		label.setText(Messages.Label_Strikethrough);
		...
		data.verticalAlignment = GridData.VERTICAL_ALIGN_BEGINNING;
		label.setLayoutData(data);
		label.setText(Messages.Label_Underline);
	...
	protected void loadFromSavedData() {
		...
		for (int i = 0; i < PreferenceConstants.PREFERENCES.length; i++) {
			...
			TextAttribute att = new TextAttribute(ColorManager.getColor(rgb),
					null, style);
			token.setData(att);
		}
		...
		for (int i = 0; i < PreferenceConstants.PREFERENCES.length; i++) {
			...
			PreferenceConverter.setValue(store,
					PreferenceConstants.PREFERENCES[i][1], att.getForeground()
							.getRGB());
			store.setValue(PreferenceConstants.PREFERENCES[i][2],
					att.getStyle());
		}
	}
